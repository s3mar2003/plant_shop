...
(function () {
		...
		findImages: function () {
			...
			if (foundLinks) {
				for (let i = 0; i < foundLinks.length; i++) {
					if (
						kadenceThemeLightbox.checkImage(foundLinks[i]) &&
						!foundLinks[i].classList.contains("kt-lightbox")
					) {
		...
		findGalleries: function () {
			...
			if (foundGalleries) {
				for (let i = 0; i < foundGalleries.length; i++) {
					var foundLinks = foundGalleries[i].querySelectorAll(
					...
					if (foundLinks) {
						for (let i = 0; i < foundLinks.length; i++) {
							if (
								...
								foundLinks[i].classList.add(
									"kt-core-gallery-lightbox"
								);
							}
						}
					}
	...
	document.addEventListener("kb-query-loaded", kadenceThemeLightbox.init);
	document.addEventListener("kadenceJSInitReload", kadenceThemeLightbox.init);
})();
